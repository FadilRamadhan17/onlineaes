...
class AES:
    def __init__(self, key):
        ...
        if len(key_bytes) < 16:
            key_bytes.extend([0] * (16 - len(key_bytes)))
    ...
    def _key_expansion(self, key):
        ...
        key_array = np.array(key).reshape(4, 4).T
        round_keys[0] = key_array
        
        for i in range(1, 11):
            temp = np.roll(round_keys[i-1][:, 3], -1)
            ...
            for j in range(4):
                if j == 0:
                    round_keys[i][:, j] = round_keys[i-1][:, j] ^ temp
                else:
                    round_keys[i][:, j] = round_keys[i-1][:, j] ^ round_keys[i][:, j-1]
    ...
    def encrypt_block(self, block):
        ...
        for round in range(1, 10):
            ...
            state = state ^ self.round_keys[round]
    ...
    def _mix_columns(self, state):
        ...
        for col in range(4):
            ...
            for row in range(4):
                val = 0
                for k in range(4):
                    val ^= self._galois_multiply(mix_matrix[row][k], column[k])
                new_column[row] = val

            new_state[:, col] = new_column
        return new_state

    def _galois_multiply(self, a, b):
        p = 0
        for _ in range(8):
            if b & 1:
                p ^= a
    ...
    def _inv_mix_columns(self, state):
        ...
        for col in range(4):
            ...
            for row in range(4):
                val = 0
                for k in range(4):
                    val ^= self._galois_multiply(inv_mix_matrix[row][k], column[k])
                new_column[row] = val
